...
static size_t adjust_request_size(size_t size, size_t align) {
    ...
    if (size) {
        ...
        /*
        ** aligned size must not exceed block_size_max or we'll go out of bounds
        ** on sl_bitmap; align_up wraps to a tiny value for sizes near SIZE_MAX,
        ** so a wrapped result must also be rejected.
        */
        if (aligned < block_size_max && aligned >= size) {
...
void* tlsf_realloc(tlsf_t tlsf, void* ptr, size_t size) {
    ...
    /* Requests with nullptr pointers are treated as malloc. */
        ...
        /*
        ** An unsatisfiable size adjusts to zero; letting zero reach
        ** block_trim_used below would split the block at size zero and
        ** corrupt its header. Fail and leave the original buffer untouched.
        */
        if (adjust == 0) {
            p = 0;
        }
        ...
        */
        else if (adjust > cursize && (!block_is_free(next) || adjust > combined)) {
            p = tlsf_malloc(tlsf, size);
            if (p) {
